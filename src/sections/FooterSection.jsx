...
const FooterSection = () => {
  ...
  return (
    <footer
      ref={footerRef}
      className="relative w-full min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 overflow-hidden touch-manipulation"
    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 min-h-screen items-center px-3 sm:px-4 lg:px-6 max-w-7xl mx-auto py-8 sm:py-12 lg:py-16">
        ...
        <div className="text-center lg:text-left flex flex-col justify-center h-full py-4 sm:py-6 lg:py-8">
            ...
            {socialLinks.map(({ icon: Icon, href, label, bgColor, hoverColor, onClick }, index) => (
              onClick ? (
                ...
                >
                  ...
                  {/* Dynamic Tooltip */}
                  <span
                    className={`absolute -top-12 sm:-top-16 left-1/2 transform -translate-x-1/2 px-3 sm:px-4 py-2 bg-black text-white text-xs sm:text-sm font-bold rounded-lg transition-all duration-300 ${isHovered === index ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                      }`}
                  >
                    {label}
                  ...
                  {/* Glow Effect */}
                  <div
                    className={`absolute inset-0 rounded-2xl bg-white/30 blur-xl transition-opacity duration-300 ${isHovered === index ? 'opacity-100' : 'opacity-0'
                      }`}
                  />
                </button>
              ) : (
          ...
          >
            ...
            <p className="text-gray-800 font-semibold text-base sm:text-lg relative z-10 transition-all duration-300 group-hover:text-gray-900">
              © {new Date().getFullYear()} <span className="text-orange-600 font-bold transition-all duration-300 group-hover:text-orange-700 group-hover:scale-105 inline-block">All Rights Reserved</span>
              <br />
